...
namespace Core.Fem
{
	...
	public class PlanarStressSolver
	{
		...
		public void SolvePlaneStress(IProgress<TaskProgress> progressReport)
		{
			...
			var counter = 0;
			for (var i = 0; i < this._mesh.NumNodes; i++)
			{
			...
			for (var i = 0; i < this._mesh.NumElements; i++)
			{
				...
				var progressPercent = (int)(100f * i / this._mesh.NumElements);
				if (progressPercent > lastProgressPercent)
				{
					lastProgressPercent = progressPercent;
		...
		/// <summary>
		/// Populates the localStiffness arrays
		/// </summary>
